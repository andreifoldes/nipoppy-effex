...
5.0.9,][]{mcflirt}. BOLD runs were slice-time corrected to 0.644s (0.5
of slice acquisition range 0s-1.29s) using \texttt{3dTshift} from AFNI
...
This mask is obtained by thresholding the corresponding partial volume
map at 0.05, and it ensures components are not extracted from voxels
containing a minimal fraction of GM. Finally, these masks are resampled
into BOLD space and binarized by thresholding at 0.99 (as in the
original implementation). Components are also calculated separately
